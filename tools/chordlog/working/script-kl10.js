...
if (measuresPerLine === 4) {
  weights = [0.36, 0.22, 0.22, 0.22];
}
else if (measuresPerLine === 6) {
  weights = [0.25, 0.15, 0.15, 0.15, 0.15, 0.15];
}
else if (measuresPerLine === 8) {
  weights = [0.20, 0.114, 0.114, 0.114, 0.114, 0.114, 0.114, 0.114];
}
else {
  weights = new Array(measuresPerLine).fill(1 / measuresPerLine);
}
...
function renderChordStaff({ chords, doubleBars = [], timeSig = "4/4", measuresPerLine = 8 }) {
  ...
  const measureW = Math.floor((usableW - 10) / measuresPerLine);
...
const measureW = Math.floor(usableW * weights[m]);
