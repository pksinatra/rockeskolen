...
<head>
  ...
  <title>Rockeskolens digitale stemmegaffel</title>
  <style>
    :root{
      --bg: #0b0b10;
...
<body>
  <div class="app">
    ...
    <section class="card">
      <header>
        <div>
          <div class="title">Rockeskolens stemmegaffel</div>
          <div class="subtitle">Ren referansetone (sinus) · Stem etter øret</div>
        </div>
        ...
        <div class="stage">
          ...
          <div class="readout">
            <div>
              ...
              <div id="noteText" class="note">A4 (konsertpitch)</div>
            </div>
            <div class="note" id="statusText">Klar</div>
          </div>

          <!-- Instrument chooser -->
          <div class="controls" style="grid-template-columns: repeat(auto-fit, minmax(90px,1fr));">
            <button class="btn instrument-btn" data-instrument="guitar">🎸 Gitar</button>
            ...
            <button class="btn instrument-btn" data-instrument="violin">🎻 Fiolin</button>
            <button class="btn instrument-btn" data-instrument="ukulele">🪕 Ukulele</button>
          </div>

          <!-- String buttons -->
          <div class="controls" id="stringButtons" style="grid-template-columns: repeat(auto-fit, minmax(60px,1fr));"></div>

          <div class="controls">
            <input id="freq" type="range" min="27.5" max="2093" step="0.1" value="440" />
            <button id="play" class="btn primary">▶ Spill</button>
            <button id="stop" class="btn">⏹ Stopp</button>
          </div>

          <div class="controls" style="grid-template-columns: 1fr 1fr 1fr;">
            <button id="octDown" class="btn" title="Samme tone én oktav dypere">– Oktav</button>
            <button id="set440" class="btn" title="Tilbakestill til A4 (440 Hz)">A4 = 440</button>
            <button id="octUp" class="btn" title="Samme tone én oktav høyere">+ Oktav</button>
          </div>
        </div>
      </div>
    </section>
    ...
    <aside class="card tip">
      ...
      <div class="content">
        <div class="badge"><span class="dot"></span><span id="tipBadge">A4 – 440 Hz</span></div>
        <p id="tipText">
          A4 (440 Hz) er standard referansetone. Stem etter øret: finn “beats” (pulsering) mellom instrumentet og tonen, og juster til pulseringen forsvinner.
        </p>
        <p class="subtitle">
          Hint: Hvis dype toner er vanskelige å høre, trykk + Oktav for å høre samme tone én oktav høyere. Du stemmer fortsatt den samme strengen.
        </p>
      </div>
      <div class="note" id="octaveStatus">Oktav: 0</div>
    </aside>
...
(() => {
  ...
  const MIN_HZ = 27.5;
const MAX_HZ = 2093.0; // C7 (trygg og kjent grense)
  ...
  function tipFor(freq, label){
    if (Math.abs(freq - 440) < 0.05) {
      return {
        badge: "A4 – 440 Hz",
        text: "A4 (440 Hz) er standard referansetone. Stem etter øret: finn pulsering (“beats”) mellom instrumentet og tonen, og juster til pulseringen forsvinner."
      };
    }
    if (Math.abs(freq - 27.5) < 0.2) {
      return {
        badge: "A0 – 27,5 Hz",
        text: "A0 (27,5 Hz) er den laveste tonen på et 88-tangenters piano. Dype frekvenser kan være vanskelig å høre – bruk gjerne + Oktav for å høre samme tone i høyere oktav."
      };
    }
    if (Math.abs(freq - 1046.5) < 0.2) {
      return {
        badge: "C6 – 1046,5 Hz",
        text: "C6 (1046,5 Hz) omtales ofte som «høye C» i vokal-verdenen. Høye frekvenser kan oppleves skarpe – hold volumet lavt."
      };
    }
    return {
      badge: `${label} – ${freq.toFixed(2)} Hz`,
      text: "Stem etter øret: når tonen matcher, blir pulseringen mellom referansen og instrumentet langsommere – og til slutt borte."
    };
  }
  ...
  function updateUI(){
    ...
    noteText.textContent = `${n.name}${n.octave}${Math.abs(hz-440)<0.05 ? " (konsertpitch)" : ""}`;
