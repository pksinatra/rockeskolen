(function () {
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    const cagedSelect = document.getElementById("cagedSelect");
...
console.log("cagedSelect:", !!cagedSelect);
...
// Skjul Quality-dropdown – brukes ikke lenger
    ...
    function fretsToMidis(frets) {
      return fretsToPlayedNotes(frets).map(function (note) {
        return note.midi;
      });
    }

    function fretsToPlayedNotes(frets) {
      const out = [];
      for (let i = 0; i < TUNING.length; i++) {
        ...
        out.push({ midi, stringIndex: i });
      }
      return out;
    }

    function normalizeCagedShape(value) {
      const raw = (value || "").toString().trim().toUpperCase();
      return ["C", "A", "G", "E", "D"].includes(raw) ? raw : null;
    }

    function getVariantCagedShape(variant, variantIndex) {
      if (!variant || typeof variant !== "object") return null;
      const direct = normalizeCagedShape(
        variant.caged || variant.cagedShape || variant.shape || variant.form
      );
      if (direct) return direct;

      const haystack = [variant.id, variant.name, variant.label, variant.source]
        .filter(Boolean)
        .join(" ")
        .toUpperCase();
      const match = haystack.match(/\b([CAGED])\s*(?:SHAPE|FORM|CAGED)\b|\bCAGED\s*([CAGED])\b/);
      if (match) return normalizeCagedShape(match[1] || match[2]);

      const fallbackOrder = ["C", "A", "G", "E", "D"];
      return Number.isInteger(variantIndex)
        ? fallbackOrder[((variantIndex % fallbackOrder.length) + fallbackOrder.length) % fallbackOrder.length]
        : null;
    }

    function getVisibleVariants(chord) {
      const variants = Array.isArray(chord?.variants) ? chord.variants : [];
      const selectedShape = cagedSelect.value || "all";
      if (selectedShape === "all") return variants;
      return variants.filter(function (variant, idx) {
        return getVariantCagedShape(variant, idx) === selectedShape;
      });
    }

    function clearFretboardPlaybackHighlights() {
      fretboardEl.querySelectorAll(".cl-note-playing").forEach(function (el) {
        el.classList.remove("cl-note-playing");
      });
    }

    function flashFretboardNote(stringIndex, delaySeconds, durationMs) {
      window.setTimeout(function () {
        const el = fretboardEl.querySelector('[data-string-index="' + stringIndex + '"]');
        if (!el) return;
        el.classList.add("cl-note-playing");
        window.setTimeout(function () {
          el.classList.remove("cl-note-playing");
        }, durationMs);
    ...
    async function ensureTone() {
      ...
      toneDrive = new Tone.Distortion(0).connect(toneLimiter);
    ...
    // ---- Datakilder ----
    function normalizeApiGuitarChord(row, requestedRoot) {
      const data = row?.data || {};
      const root = (row?.root_note || data?.root || requestedRoot || "").toString().trim();
      const symbol = (data?.symbol || data?.name || row?.name || row?.slug || "").toString().trim();
      const slug = (data?.slug || row?.slug || symbol || root).toString().trim().toLowerCase();
      const variants = Array.isArray(data?.variants) ? data.variants : [];
      if (!root || !variants.length) return null;

      const rootSlug = root.toLowerCase().replace(/[^a-g#b]/g, "");
      const normalizedSlug = slug.replace(/[^a-z0-9#b()+]/g, "");
      let suffix = normalizedSlug.startsWith(rootSlug)
        ? normalizedSlug.slice(rootSlug.length)
        : normalizedSlug;
      if (!suffix || suffix === "major") suffix = "maj";
      if (suffix === "m" || suffix === "minor") suffix = "min";
      const parsed = (data?.family && data?.quality)
        ? { family: data.family, quality: data.quality }
        : parseChordType(suffix);
      ...
      return {
        ...
        variants,
        slug,
        symbol: symbol || row?.name || slug,
        bookSymbol: data?.bookSymbol || "",
        aliases: Array.isArray(data?.aliases) ? data.aliases : [],
        name: row?.name || symbol || slug
      };
    }

    // ---- Filnavn basert på rot ----
    function getChordApiUrl(root) {
      const url = `/api/music.php?type=guitar_chord&instrument=guitar&root=${encodeURIComponent(root)}`;
      fileNameInfoEl.textContent = url;
      return url;
    }

    async function loadDataForRoot(root) {
      const key = root.toString().trim().toUpperCase();
      if (jsonCache[key]) return jsonCache[key];

      const url = getChordApiUrl(root);
      const res = await fetch(url, { credentials: "same-origin" });
      if (!res.ok) {
        throw new Error("Klarte ikke å laste API eller " + url + " (" + res.status + ")");
      }
      const payload = await res.json();
      if (!payload?.success || !Array.isArray(payload.results)) {
        throw new Error("Ugyldig svar fra music API");
      }

      const chords = payload.results
        .map((row) => normalizeApiGuitarChord(row, root))
        .filter(Boolean);
    ...
    function rootMatchesChord(ch, root) {
      const wanted = (root || "").toString().trim().toUpperCase();
      const chordRoot = (ch.root || "").toString().trim().toUpperCase();
      if (chordRoot === wanted) return true;
      if (Array.isArray(ch.aliases) && ch.aliases.some(alias => (alias || "").toString().trim().toUpperCase() === wanted))
        return true;
      return false;
    }
...
function updateAvailabilityForRoot(data, root) {
  ...
  if (!available.has(extSelect.value)) {
    ...
    function renderFretboard(variant) {
      ...
      for (let i = 0; i < strings; i++) {
        ...
        if (fret === 0) {
          ...
          circle.setAttribute("data-string-index", String(i));
        ...
        dot.setAttribute("data-string-index", String(i));
        svg.appendChild(dot);
    ...
    function populateVariants(chord) {
      ...
      const variants = getVisibleVariants(chord);
      if (!variants.length) {
        const opt = document.createElement("option");
        opt.value = "0";
        opt.textContent = "No variants for this CAGED shape";
        variantSelect.appendChild(opt);
        variantSelect.disabled = true;
        return;
      }
      ...
      variants.forEach(function (v) {
        const idx = chord.variants.indexOf(v);
        const opt = document.createElement("option");
        opt.value = String(idx);
        const pos = v.position || 1;
        const shape = getVariantCagedShape(v, idx);
        opt.textContent =
          (idx === 0 ? "Standard " : "Variant " + (idx + 1) + " ") +
          "(pos " + pos + (shape ? ", " + shape + " shape" : "") + ", id: " + v.id + ")";
        variantSelect.appendChild(opt);
      });
    }
    ...
    function updateHeader(chord, variant) {
      ...
      const shape = getVariantCagedShape(variant, parseInt(variantSelect.value || "0", 10));
      const bits = [];
      if (shape) bits.push("CAGED: " + shape + " shape");
      if (variant && variant.source) bits.push("Source: " + variant.source);
      variantInfoEl.textContent = bits.join(" · ");
    }
    ...
    async function updateChordFromUI() {
      ...
      ///          jjjjjjjjjjjj
const chordType = extSelect.value;
...
try {
  ...
  if (variantSelect.disabled) {
    currentVariant = null;
    fretboardEl.textContent =
      "Ingen variant for valgt CAGED-form.";
    updateHeader(chord, null);
    return;
  }
...
function onVariantChange() {
  ...
  logDebug({
    ...
    cagedSelect.addEventListener("change", updateChordFromUI);
    ...
    playBtn.addEventListener("click", async function () {
      try {
        ...
        const playedNotes = fretsToPlayedNotes(currentVariant.frets);
        ...
        const freqs = playedNotes.map(function (note) {
          return 440 * Math.pow(2, (note.midi - 69) / 12);
        });

        const now = Tone.now() + 0.05;
        const sustainMs = Number(document.getElementById("sustain")?.value || 80);
        const volumePct = Number(document.getElementById("volume")?.value || 78);
        const drivePct = Number(document.getElementById("drive")?.value || 0);
        const dur = Math.max(0.2, sustainMs / 100);
        toneSynth.set({ volume: -24 + (volumePct / 100) * 20 });
        if (toneDrive) toneDrive.distortion = Math.min(0.9, drivePct / 100); // subtle
        const strumMs = 40; // strum-delay mellom strenger

        // Enkel strum nedover (6 -> 1)
        clearFretboardPlaybackHighlights();
        freqs.forEach(function (f, i) {
          const t = now + i * (strumMs / 1000);
          toneSynth.triggerAttackRelease(f, dur, t, 0.95);
          flashFretboardNote(playedNotes[i].stringIndex, 0.05 + i * (strumMs / 1000), Math.max(160, strumMs + 180));
        });
      } catch (err) {
