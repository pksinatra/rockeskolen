(function () {
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    function ensureProBox() {
      ...
      (header && header.parentNode ? header.parentNode : card).insertBefore(
        a,
        header ? header.nextSibling : card.firstChild
      );
    }
    ...
    function showProBox(msg) {
      ...
      const text = document.getElementById("proBoxText");
  ...
  if (extSelect.selectedOptions.length && extSelect.selectedOptions[0].hidden) {
    ...
    extSelect.addEventListener("change", function () {
      if (!IS_PRO && PRO_EXTENSIONS.has(extSelect.value)) {
        showProBox("11, 13 og Altered er tilgjengelig i Pro. Klikk for å oppgradere.");
        extSelect.value = "none";
        updateChordFromUI();
        return;
      }
