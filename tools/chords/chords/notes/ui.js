...
let useKeySignature = urlParams.get("mode") === "key";
...
function fitPiano() {
  const piano = document.getElementById("piano");
  const keyboard = piano ? piano.querySelector(".keyboard") : null;

  if (!piano || !keyboard) return;

  const availableWidth = piano.clientWidth - 40;
  const naturalWidth = 720;
  const scale = Math.min(1, availableWidth / naturalWidth);

  keyboard.style.setProperty("--piano-scale", scale);
  piano.style.height = (140 * scale + 40) + "px";
}

window.addEventListener("resize", fitPiano);
