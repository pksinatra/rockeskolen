console.log("ChordLink Guitar Scales loaded");
...
const CL_IS_LOGGED_IN = window.CL_IS_LOGGED_IN === true;
let lastAllowedScale = "major";

function getUserTier(){
  if(CL_IS_PRO) return "pro";
  if(CL_IS_LOGGED_IN) return "free";
  return "guest";
}
...
function isScaleAllowed(scaleKey){
  ...
  if(tier === "guest"){
    return ["major","minor_pent"].includes(scaleKey);
  }

  if(tier === "free"){
    return FREE_SCALE_KEYS.includes(scaleKey);
  }
...
function noteToFreq(note){
  ...
  const midi = 52 + (idx < 0 ? 0 : idx); // around E3
  return 440 * Math.pow(2, (midi - 69) / 12);
}
function getAudioCfg(){
  return {
    wave: $("soundType")?.value || "triangle",
    sustain: Math.max(0.2, Number($("sustain")?.value || 85) / 100),
    volume: Math.max(0, Math.min(1, Number($("volume")?.value || 74) / 100)),
    drive: Math.max(0, Math.min(0.45, Number($("drive")?.value || 0) / 100))
  };
}
function playScale(){
  const root = $("rootSelect")?.value || "C";
  const scaleKey = $("scaleSelect")?.value || "major";
  const notes = buildScale(root, scaleKey);
  if (!notes?.length) return;
  const cfg = getAudioCfg();
  const seq = [...notes, notes[0]];
  ...
  seq.forEach((n, i)=>{
    ...
    const k = Math.floor(cfg.drive * 200);
    const curve = new Float32Array(256);
    for(let x=0; x<256; x++){
      const v = (x*2/255)-1;
      curve[x] = ((3+k)*v*20*Math.PI/180)/(Math.PI+k*Math.abs(v));
    }
    shaper.curve = curve;
    osc.type = cfg.wave;
    osc.frequency.value = noteToFreq(n);
    pre.gain.value = 1 + cfg.drive * 2;
    gain.gain.setValueAtTime(0, t);
    gain.gain.linearRampToValueAtTime(0.24 * cfg.volume, t + 0.015);
    gain.gain.exponentialRampToValueAtTime(0.0008, t + cfg.sustain);
    osc.connect(pre); pre.connect(shaper); shaper.connect(post); post.connect(gain); gain.connect(AC.destination);
    osc.start(t); osc.stop(t + cfg.sustain + 0.03);
  });
}
...
async function loadScales() {
  ...
  const url = "/api/music.php?type=scale&instrument=general";
  ...
  if (!payload?.success || !Array.isArray(payload.results)) {
    throw new Error("API returned invalid scale payload.");
  }

  SCALE_DATA = {};
  for (const row of payload.results) {
    const key = row.slug || row.name;
    if (!key || !row.data) continue;
    SCALE_DATA[key] = {
      name: row.name,
      name_no: row.name,
      intervals: row.data.intervals || row.data.steps || [],
      ...row.data
    };
  }

  if (!Object.keys(SCALE_DATA).length) {
    throw new Error("No scales found in music API.");
  }
}
...
function renderGuitar(root, scaleKey) {
  const wrap = $("guitarboard"); // ✅ ALLTID først

  // 👇 rydde gammel overlay
  wrap.classList.remove("blurred");

  const oldOverlay = wrap.querySelector(".pro-overlay");
...
/*
  setStatus(`
  🔒 The scale ${formatNote(root)} ${scaleName} is only available for Pro members<br>
<a href="/members/upgrade.php" target="_top" style="color:#4dc3ff; font-weight:600;">
  Upgrade to Pro
</a>
  `);
*/
wrap.innerHTML = `
  <div class="msg">
    🔒 The scale <strong>${formatNote(root)} ${scaleName}</strong> 
    is only available for Pro members<br><br>
    <a href="/members/upgrade.php" target="_top" class="pro-cta pro-cta--promo">
      Upgrade to Pro
    </a>
  </div>
`;
...
if(viewMode === "open"){
  ...
  if (!scale) {
    wrap.innerHTML = `<div class="msg error">Could not find scale: ${scaleKey}</div>`;
    return;
  }
  ...
  const svg = createSvgEl("svg", {
    ...
    class: "gscale-svg",
    "aria-label": "Guitar neck - Scale view"
  });
...
function updateScaleView() {
  ...
  if (!isScaleAllowed(scaleKey)) {
    showProGate(root, scaleKey);
    return; // ❗ IKKE revert dropdown
  }

  // 👇 lagre som siste gyldige
  lastAllowedScale = scaleKey;
...
function toggleLabels() {
  ...
  if (btn) {
    btn.textContent = SHOW_DEGREES ? "Tones" : "Intervals";
  }
  updateScaleView();
}
function showProGate(root, scaleKey){
  const wrap = $("guitarboard");
  const scale = SCALE_DATA[scaleKey] || {};
  const scaleName = scale.name_no || scale.name || scaleKey;

  setStatus(`
    🔒 ${formatNote(root)} ${scaleName} is a PRO scale<br>
    <a href="/members/upgrade.php" target="_top" style="color:#4dc3ff; font-weight:600;">
      Upgrade to Pro
    </a>
  `);
  ...
  wrap.innerHTML += `
    <div class="pro-overlay">
      <div class="pro-box">
        ...
        Only available in PRO<br><br>
        <a href="/members/upgrade.php" target="_top" class="upgrade-btn">
          Upgrade
        </a>
      </div>
    </div>
...
function isRootAllowed(root){
  ...
  if(tier === "guest"){
    return ["C","F","G"].includes(root);
  }

  if(tier === "free"){
    return ["C","D","E","F","G","A"].includes(root);
  }
...
$("noteStyleBtn")?.addEventListener("click", toggleNoteStyle);
    ...
    updateScaleView();
    lastAllowedScale = $("scaleSelect")?.value || "major";
  } catch (err) {
    console.error(err);
    const wrap = $("guitarboard");
    if (wrap) {
      wrap.innerHTML = `<div class="msg error">Could not load scales. ${err.message}</div>`;
    }
    setStatus("Could not load scales.", true);
  }
}
