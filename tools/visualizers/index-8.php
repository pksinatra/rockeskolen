...
<title>Visualizer V4.2.1 Presets</title>
...
<div id="status">
    <div><strong>V4.2.1 Presets</strong></div>
...
function buildScaleSequence(root, scaleKey, octave = 4) {
    ...
    intervals.forEach((interval, idx) => {
        ...
        const wrapped = rootIndex + interval >= 12 ? 1 : 0;
        seq.push({
            note: noteNames[noteIndex] + (octave + wrapped),
            time: idx * 0.5,
            duration: 0.45,
            velocity: 108
        });
    ...
    // legg til oktaven på toppen hvis det gir mening
    if (intervals.length > 0 && scaleKey !== "chromatic") {
        seq.push({
            note: root + (octave + 1),
            time: intervals.length * 0.5,
            duration: 0.7,
            velocity: 112
        });
    }
...
function updatePresetStatus() {
    ...
    const presetLabel = presetSelect.options[presetSelect.selectedIndex]?.textContent || "";
